...
public class Parqueadero {
    ...
    private Map<Integer, Ticket> todosLosTickets = new HashMap<>();// Tarifa por hora
    ...
    // Metodo para agregar un vehículo y asignar un espacio de parqueo
    public EspacioParqueo agregarVehiculo(Vehiculo vehiculo) {
        for (EspacioParqueo espacio : espacios) {
            if (espacio.estaDisponible()) {
                ...
                todosLosTickets.put(ticket.getIdTicket(), ticket);
    ...
    public EspacioParqueo agregarVehiculoConTicket(Ticket ticket) {
    ...
    public Ticket getTicketPorId(int id) {
        return todosLosTickets.get(id);
    }


    public Ticket darSalidaVehiculoPorIdTicket(int idTicket, LocalDateTime horaSalida) {
        for (EspacioParqueo espacio : espacios) {
            if (!espacio.estaDisponible()) {
                Ticket ticket = espacio.getTicket();
                if (ticket != null && ticket.getIdTicket() == idTicket) {
                    Vehiculo vehiculo = ticket.getVehiculo();
                    double tarifa = tarifas.getOrDefault(vehiculo.getTipo(), 1000.0);
                    ticket.registrarSalida(horaSalida, tarifa);
                    espacio.liberar();
                    return ticket;
                }
        ...
        return null; // No se encontró ticket con ese ID
    }



    // Metodo para obtener el número de espacios disponibles
    ...
    // Metodo para obtener el número de espacios ocupados
